...
class TestDataAugmentor(unittest.TestCase):
    ...
    def test_initialization(self):
        ...
        augmentor = DataAugmentor(self.df, class_column='Species')
        self.assertTrue(augmentor.df.equals(self.df))
        self.assertEqual(augmentor.class_column, 'Species')
        self.assertEqual(augmentor.image_column, 'Image')
        self.assertEqual(augmentor.id_column, 'UniqueID')
        self.assertEqual(augmentor.threshold, 20)

    def test_get_rare_classes(self):
        """ Test the get_rare_classes method returns the expected classes. """
        augmentor = DataAugmentor(self.df, class_column='Species', threshold=3)
        rare_classes = augmentor.get_rare_classes()
    ...
    def test_binary_to_pil_and_back(self):
        ...
        pil_img = augmentor.binary_to_pil(original_blob)
        self.assertIsInstance(pil_img, Image.Image)

        new_blob = augmentor.pil_to_binary(pil_img)
    ...
    def test_augment_rare_classes_adds_images(self):
        ...
        augmentor = DataAugmentor(self.df, class_column='Species', threshold=3)
        augmented_df = augmentor.augment_rare_classes(self.dummy_augmentation, num_augments_per_image=2)
    ...
    def test_no_augmentation_for_common_classes(self):
        ...
        augmentor = DataAugmentor(self.df, class_column='Species', threshold=2)
        augmented_df = augmentor.augment_rare_classes(self.dummy_augmentation, num_augments_per_image=1)
