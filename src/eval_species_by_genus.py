...
class EvalSpeciesByGenus:
    ...
    def get_species(self, caud=None, dors=None, fron=None, late=None):
        ...
        if caud:
            ...
            input_order.append("caud")
        if dors:
            all_inputs.append(dors)
            input_order.append("dors")
        if fron:
            all_inputs.append(fron)
            input_order.append("fron")
        if late:
            all_inputs.append(late)
            input_order.append("late")

        count = 0
        for i in all_inputs:
            image = self.transform_input(i, self.transformations[input_order[count]])
